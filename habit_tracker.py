...
class DashboardService:
    ...
    def get_statistics(self, user_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                """SELECT COALESCE(SUM(hc.completion_count), 0) AS count FROM HABIT_COMPLETION hc
                   INNER JOIN HABITS h ON h.habit_id = hc.habit_id
                   WHERE h.user_id = %s AND h.status = 'active' AND hc.completed = 1
            ...
            cursor.execute(
                """SELECT COALESCE(SUM(hc.completion_count), 0) AS count FROM HABIT_COMPLETION hc
                   INNER JOIN HABITS h ON h.habit_id = hc.habit_id
                   WHERE h.user_id = %s AND h.status = 'active' AND hc.completed = 1
                   AND hc.completion_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 29 DAY) AND CURDATE()""",
                (user_id,),
            )
...
class HabitTracker:
    ...
    def get_day(self, user_id: int, requested_date: str) -> dict[str, Any]:
        ...
        try:
            ...
            for record in cursor.fetchall():
                state = "completed" if record["completed"] else ("upcoming" if selected_date > date.today() else "missed")
                habits.append({**record, "completed": bool(record["completed"]), "completion_count": int(record["completion_count"] or 0), "notes": record["notes"] or "", "state": state})
    ...
    def record_completion(self, user_id: int, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            cursor.execute("SELECT habit_id FROM HABITS WHERE habit_id=%s AND user_id=%s AND status='active'", (habit_id, user_id))
            if not cursor.fetchone(): raise ValueError("Habit not found or inactive.")
            ...
            if existing:
                cursor.execute("UPDATE HABIT_COMPLETION SET completed=%s, completion_count=%s, notes=%s WHERE completion_id=%s", (completed, completion_count if completed else 0, notes, existing[0]))
            else:
                cursor.execute("INSERT INTO HABIT_COMPLETION (habit_id, completion_date, completed, completion_count, notes) VALUES (%s,%s,%s,%s,%s)", (habit_id, completion_date, completed, completion_count if completed else 0, notes))
            connection.commit(); logger.info("Completion recorded: habit_id=%s user_id=%s", habit_id, user_id)
        except Error as error:
...
class Analytics:
    ...
    def calculate(self, user_id: int) -> list[dict[str, Any]]:
        results=[]
        for habit in self._habit_rows(user_id):
            completion_rows=self._completion_rows(habit["habit_id"]); dates=[row["completion_date"] for row in completion_rows]
            completed=sum(int(row["completion_count"] or 0) for row in completion_rows)
            expected=DashboardService._expected_completions(habit["goal_type"], int(habit["target_count"]), habit["start_date"])
            current,longest=DashboardService._streaks(dates); days=max(1,(date.today()-habit["start_date"]).days+1)
            ...
            self._save(result); results.append(result)
...
class ChartGenerator:
    ...
    def generate(self, user_id: int) -> dict[str, str]:
        ...
        labels=[r["month"] for r in monthly] or [date.today().strftime("%Y-%m")]; values=[int(r["completed"]) for r in monthly] or [0]; plt.figure(figsize=(7,3.5)); plt.plot(labels,values,marker="o",color="#f39b58"); plt.title("Monthly trend"); plt.ylabel("Completions"); plt.grid(axis="y",alpha=.25); plt.tight_layout(); plt.savefig(paths["monthly"],dpi=140); plt.close(); logger.info("Charts generated for user_id=%s",user_id)
